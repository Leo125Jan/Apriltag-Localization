							...
							double desired_yaw, float f_x, float f_y)
{
	...
	iris_2_pub = nh.advertise<std_msgs::Int64>("/iris2/go", 1000);
	land_sub = nh.subscribe("/tag_detections", 1000, LandCallback);
	i_ode_sub = nh.subscribe("/iris/odometry_sensor1/pose", 1000, i_ode_Callback);
	motor_publisher = nh.advertise<mav_msgs::Actuators>("/iris/command/motor_speed", 10);
	...
	ROS_INFO("Publish iris2 Start message");

	std_msgs::Int64 iris2_go;
	iris2_go.data = 1;

	iris_2_pub.publish(iris2_go);
...
{
	// Move to firefly
	ROS_INFO("Waitting for firefly position message");
    ...
    float f_x = Real_x_h, f_y = Real_y_h;
